...
FROM world.port_index
WHERE COUNTRY = 'VN'
ORDER BY ABS(LONGITUDE - (SELECT LONGITUDE FROM world.port_index WHERE PORT_NAME = 'DA NANG' AND COUNTRY = 'VN'))
LIMIT 5;
...
SELECT *
FROM world.port_index
WHERE REGION_NO = (SELECT REGION_NO FROM world.port_index WHERE PORT_NAME = 'TERN ISLAND' AND COUNTRY = 'US');
...
FROM world.port_index
WHERE SQRT(POWER(LATITUDE - (SELECT LATITUDE FROM world.port_index WHERE PORT_NAME = 'DA NANG' AND COUNTRY = 'VN'), 2) + POWER(LONGITUDE - (SELECT LONGITUDE FROM world.port_index WHERE PORT_NAME = 'DA NANG' AND COUNTRY = 'VN'), 2)) <= 500;
...
FROM world.port_index
GROUP BY COUNTRY
HAVING COUNT(*) = SUM(CASE WHEN LONG_HEMI = 'E' THEN 1 ELSE 0 END);
